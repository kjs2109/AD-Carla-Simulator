...
ad_system = os.path.abspath("ad_system") 
print(ad_system)

# 현재 PYTHONPATH에 경로가 없다면 추가
if ad_system not in sys.path:
    sys.path.append(ad_system)
...
class ScenarioRunner(object):
    ...
    def _load_and_run_scenario(self, config):
        ...
        if self._args.agent:
            ...
            try:
                self.agent_instance = getattr(self.module_agent, agent_class_name)(self.ego_vehicles[0], self._args.agentConfig) 
                print("Agent {} loaded".format(agent_class_name))
